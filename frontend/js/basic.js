...
function forgotSubmitted(event)
{
    event.preventDefault();
    document.getElementsByTagName('p').innerHTML = '';
    axios.post('http://localhost:4000/password/forgotpassword', {email:event.target.email.value})
        .then((result)=>{
            console.log('result...', result);
            document.getElementById('error-msg').innerHTML = result.data.toString();
        })
        .catch(error=>{
            console.log(error);
            document.getElementById('error-msg').innerHTML = error.response.data.toString();
        });
}